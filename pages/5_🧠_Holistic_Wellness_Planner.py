...
def get_personalized_default_content(task_type, goal, user_data, is_full_fallback=False):
    ...
    if task_type == "stress_task":
        ...
        if goal == "Increase Water Intake":
            default_tip = "Make it easy to access water by keeping a bottle nearby. Consider setting hourly reminders or pairing water intake with existing daily activities like meals or breaks."
        elif goal == "Eat More Vegetables":
...
if st.button("Submit Question 🗣️", key="submit_query_btn"):
    ...
    if not question:
        ...
        qna_placeholder_fallback_message = f"Content unavailable. For more specific guidance, please ensure your wellness profile is filled out. In the meantime, general principles apply."
        ...
        try:
            ...
            if not cleaned_response or qna_placeholder_fallback_message in raw_response_text:
                st.session_state.qa_response = f"<div class='result-box'>{qna_placeholder_fallback_message}</div>"
            else:
                st.session_state.qa_response = f"<div class='result-box'>{raw_response_text}</div>"
            ...
            st.session_state.qa_response = f"<div class='result-box'>An error occurred while fetching the response. Please try again.</div>"
            st.markdown(st.session_state.qa_response, unsafe_allow_html=True)
